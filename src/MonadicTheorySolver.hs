...
makeBvType n = SMT.tBits (toInteger n) -- replicate (fromInteger $ toInteger n) SMT.tBool

bvLength :: SMT.SExpr -> Integer
bvLength (SMT.List [_, (SMT.Atom "BitVec"), (SMT.Atom si)]) =  read si
...
declareProdFuncions s numPreds bvType funs maxArity = error "TODO productions"
  -- forM_ funs $ \f -> do
  ...
  --   --         [0 .. (arity f) - 1]
  --   let prodTos :: [BitVector] = [nameToBits numPreds ("toSymb" ++ show i) | i <- [0 .. (arity f) - 1] ]
...
withNForalls vars numBits comp = do
  ...
  where
    varTypes = [SMT.List [bitList bv, makeBvType numBits] | bv <- vars]
...
enumerateProductions s bvType funs
 ...
 = do
  ...
  where
    productionVal i = nameToBits (bvLength bvType) $ argName i
    helper ::
         Set.Set BitVector
...
declareOrDefineFuns s numPreds bvType state exprs = do
  ...
  forM_ funs $ \f@(VecFun fName arity) -> do
    ...
    case (length funBodies, numVars ) of
      ...
      (_,0) -> defineFun s (Fun fName) argPairs bvType (concatBits numFuns funBodies)
      _ -> defineFun s (Fun fName) argPairs bvType (concatBV (varFun $$$ allArgs) (concatBits numFuns funBodies))
  return ()
...
declareDomain s numPreds bvType boolDomPreds boolDomArgName
 ...
 = do
  ...
  SMT.defineFun
    s
    "domain"
    (zip [boolDomArgName] [bvType])
    SMT.tBool
    (boolDomPreds `SMT.and`  ((Fun "domainToBeSolved") $$ [SMT.Atom boolDomArgName]))
